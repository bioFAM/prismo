...
def load_mouse_fibroblasts() -> MuData:
    ...
    var = pd.merge(pd.DataFrame(index=c57.columns), gene_metadata, left_index=True, right_index=True, how="left")
    ...
    obs["active_X"] = np.select(
        condlist=[X_ratio < 0.3, (X_ratio >= 0.3) & (X_ratio <= 0.7), X_ratio > 0.7], choicelist=["c57", np.nan, "cast"]
    )
